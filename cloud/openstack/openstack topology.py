...
from nova.virt.disk_g.vfs import api as vfs
...
class VFSGuestFS(vfs.VFS):
    ...
    def setup_os_root(self, root):
        ...
        for mount in mounts:
            # add by lyz, ingnore no root file sys
            if root != mount[1]:
                continue
    ...
    def make_path(self, path):
        LOG.debug("Make directory path=%(path)s" % locals())
        path = self._canonicalize_path(path)
        self.handle.mkdir_p(path)

    def append_file(self, path, content):
        LOG.debug("Append file path=%(path)s" % locals())
        path = self._canonicalize_path(path)
        self.handle.write_append(path, content)

    def replace_file(self, path, content):
        LOG.debug("Replace file path=%(path)s" % locals())
        import sys  
        reload(sys)  
    ...
    def read_file(self, path):
        LOG.debug("Read file path=%(path)s" % locals())
        path = self._canonicalize_path(path)
        return self.handle.read_file(path)

    def has_file(self, path):
        LOG.debug("Has file path=%(path)s" % locals())
        path = self._canonicalize_path(path)
        try:
    ...
    def set_permissions(self, path, mode):
        LOG.debug("Set permissions path=%(path)s mode=%(mode)s" % locals())
        path = self._canonicalize_path(path)
    ...
    def set_ownership(self, path, user, group):
        ...
        LOG.debug("chown uid=%(uid)d gid=%(gid)s" % locals())
